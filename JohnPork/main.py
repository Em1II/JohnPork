...
from scripts import getNameById, prettyTime, getaPorkSong, delete_article
import datetime
...
app.jinja_env.globals.update(getNameById=getNameById, prettyTime=prettyTime, getaPorkSong=getaPorkSong, delete_article=delete_article)
...
@app.route("/api/delete_article/<int:id>", methods=["DELETE", "GET"])
@login_required
def delete_article(id):
    ...
    try:   
        ...
        if current_user.rank == 1 or current_user.id == article.creator_id:
            session.delete(article)
            session.commit()
            return redirect("/")
        else:
            return render_template("index.html", messages=["Самый умный, да?"], title="Самый умный, да?")
    except:
        return redirect("/")
...
def register():
    db = db_session.create_session()
    form = RegisterForm()
    if db.query(User).filter(User.email==form.email.data).all():
        flash('Такой email уже занят')
        return redirect("/register")
    elif form.name.data and form.name.data.lower() == "crabgod":
        flash('Тут может быть только один CrabGod')
        return redirect("/register")
    ...
    return render_template('register.html', title='Авторизация прервана по установленной, но не исправленной причине', form=form)
